...
def compute_lr(ls_simi, pid, logit_scale, image_id=None, factor=0.3, epsilon=1e-8):
    """
    Similarity Distribution Matching
    """
    batch_size = ls_simi.shape[0]
    pid = pid.reshape((batch_size, 1)) # make sure pid size is [batch_size, 1]
    pid_dist = pid - pid.t()
    labels = (pid_dist == 0).float()
    ...
    ls_simi = logit_scale * ls_simi
    # ls_B_A_simi = logit_scale * ls_B_A_simi

    # normalize the true matching distribution
    labels_distribute = labels / labels.sum(dim=1)

    i2t_pred = F.softmax(ls_simi, dim=1)
    i2t_loss = i2t_pred * (F.log_softmax(ls_simi, dim=1) - torch.log(labels_distribute + epsilon))
...
def compute_id(image_logits, thermal_logits, labels):
    """
    Instance loss proposed at http://arxiv.org/abs/1711.05535
    """
    criterion = nn.CrossEntropyLoss(reduction="mean")

    loss = criterion(image_logits, labels) + criterion(thermal_logits, labels)
...
def compute_infonce(image_features, text_features, logit_scale=0.07): # new
    """
    image-text contrastive (ITC) loss, InfoNCE
    """
    batch_size = image_features.shape[0]
