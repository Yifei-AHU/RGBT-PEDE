...
class TransformerDecoderBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.attn_mask = attn_mask  # for masked self-attention

    def masked_self_attention(self, x: torch.Tensor):
        mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        return self.self_attn(x, x, x, attn_mask=mask, need_weights=False)[0]

    def cross_attention(self, x: torch.Tensor, memory: torch.Tensor):
        return self.cross_attn(x, memory, memory, need_weights=False)[0]

    def forward(self, x: torch.Tensor, memory: torch.Tensor):
        # x: (T, B, C), memory: (S, B, C)
        x = x + self.masked_self_attention(self.ln_1(x))
        x = x + self.cross_attention(self.ln_2(x), memory)
        x = x + self.mlp(self.ln_3(x))
        return x
    
class Gran_Decoder(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None):
        super().__init__()
        self.width = width
        self.layers = layers
        self.resblocks = nn.ModuleList(
            [TransformerDecoderBlock(width, heads, attn_mask) for _ in range(layers)]
        )
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisionTransformer(
                ...
                width=vision_width,
                layers=vision_layers,  # vision_layers
                heads=vision_heads,
                output_dim=embed_dim
            )

            # self.rgb_visual = VisionTransformer(
            #     input_resolution=image_resolution,
            #     patch_size=vision_patch_size,
            #     stride_size=stride_size,
            #     width=vision_width,
            ...
            # self.t_visual = VisionTransformer(
            #     input_resolution=image_resolution,
            #     patch_size=vision_patch_size,
            #     stride_size=stride_size,
            #     width=vision_width,
    ...
    def forward(self, rgb_image, t_image, text, random_mask_text1, random_mask_text2, mask_nouns_text, mask_color_text):
        rgb_image_features = self.encode_image(rgb_image)
        t_image_features = self.encode_image(t_image)
        text_features = self.encode_text(text)

        with torch.no_grad():
            no_color_random_mask_text_features = self.encode_text(random_mask_text1) # 全局和T对应的 随机掩码的无颜色文本
            mask_nouns_text_features = self.encode_text(mask_nouns_text)
            mask_color_text_features = self.encode_text(mask_color_text)
        
        complete_random_mask_text_features = self.encode_text(random_mask_text2) # 全局和RGB对应的，随机掩码的完整文本

        return rgb_image_features, t_image_features, text_features, no_color_random_mask_text_features, complete_random_mask_text_features, mask_nouns_text_features, mask_color_text_features
    
    # def forward(self, rgb_image, t_image, text, mask_nouns_text, mask_color_text):
    #     rgb_image_features = self.encode_image(rgb_image)
    #     t_image_features = self.encode_image(t_image)
    #     text_features = self.encode_text(text)

    #     with torch.no_grad():
    #         mask_nouns_text_features = self.encode_text(mask_nouns_text) # 掩码名词的和T对应 不需要的梯度回传
        
    #     mask_color_text_features = self.encode_text(mask_color_text) # 掩码颜色和其它 需要梯度回传优化

    #     return rgb_image_features, t_image_features, text_features, mask_nouns_text_features, mask_color_text_features
...
def build_T_Encoder_from_openai_pretrained(name: str, image_size: Union[int, Tuple[int, int]], stride_size: int, jit: bool = False, download_root: str = None):
    """Load a CLIP model
